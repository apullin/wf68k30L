...
always_ff @(posedge CLK) begin : data_fault_info
    ...
    if (!BUSY_EXH && !SSW_FROZEN) begin // Do not alter during exception processing.
        // An external handler decodes this, so the SIZE field must match the
        // SIZ1/SIZ0 pins rather than this design's internal OP_SIZE codes.
        // um_size_code returns SIZE_N[1:0] for the same operand, so the SSW
        // agrees with what the bus actually drove.
        SIZEVAR = um_size_code(OP_SIZE);
