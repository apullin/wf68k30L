...
logic [MMU_DESC_SHADOW_SET_BITS-1:0] inv_set_idx;
...
for (genvar way_i = 0; way_i < MMU_DESC_SHADOW_WAYS; way_i = way_i + 1) begin : gen_shadow_way_ram
    assign SHADOW_WR_EN[way_i] = WR_EN && WR_VALID && (wr_way_sel == way_i[MMU_DESC_SHADOW_WAY_BITS-1:0]);
...
always_ff @(posedge CLK) begin : shadow_state
    ...
    if (RESET_CPU) begin
        ...
        if (WR_EN && WR_VALID) begin
            ...
            MMU_DESC_SHADOW_V[wr_set_idx] <= '0;
        end

        if (INV_EN)
            MMU_DESC_SHADOW_V[inv_set_idx] <= '0;
    end
end
