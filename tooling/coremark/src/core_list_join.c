...
core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed)
{
    ...
    ee_u32 raw_size = (blksize / per_item);
    ee_u32 size     = (raw_size > 2) ? (raw_size - 2) : 1;
#ifdef COREMARK_LIST_ITEMS
    if (size > (ee_u32)COREMARK_LIST_ITEMS)
        size = (ee_u32)COREMARK_LIST_ITEMS;
    if (size == 0)
        size = 1;
#endif
    /* to accommodate systems with 64b pointers, and make sure same code is
     * executed, set max list elements */
